...
(function (global, factory) {
  ...
  function treeWillUnMount(virtualNode) {
    if (isObject(virtualNode)) {
      if (isComponent(virtualNode.type)) {
        ...
        var children = isArray(virtualNode) ? virtualNode : virtualNode.props.children;
  ...
  function mountArrays(elementDefinition, el) {
    var _loop2 = function _loop2(j) {
      ...
      mount(singleElementDefinition, el, function () {
        return el.appendChild(singleElementDefinition.realDOM);
      });
    };
  ...
  function render(virtualNode) {
    ...
    if (isComponent(virtualNode.type)) {
      ...
      var _newNodeDefinition = render(componentInternals.virtualNode);
  ...
  function diffComponents(oldComponent, newComponent, isVOldNodeComponent, isVNewNodeComponent) {
    ...
    if (isVOldNodeComponent && isVNewNodeComponent) {
      ...
      if (oldComponent.type === newComponent.type) {
        ...
        var update = updateComponent(oldComponent, newComponent.props, nextState);
        return applyComponentUpdate(update, function (patch, snapshot) {
          return function (node) {
      ...
      var _diffPatch = diff(vOldNodeInstanceInternals.virtualNode, _vNewNodeInstance._internals.virtualNode);

      componentWillUnMount(oldComponent);
      return function (node) {
        _vNewNodeInstance.onComponentWillMount(node, node.parentNode);

        var patchedChild = _diffPatch ? _diffPatch(node) : vOldNodeInstanceInternals;
        ...
        _vNewNodeInstance.onComponentMount(realDOMofNewComponent, realDOMofNewComponent.parentNode);
    ...
    if (isVOldNodeComponent && !isVNewNodeComponent) {
      ...
      var _diffPatch2 = diff(_vOldNodeInstanceInternals.virtualNode, newComponent);
    ...
    return function (node) {
      ...
      var patchedChild = diffPatch ? diffPatch(node) : {
        virtualNode: oldComponent,
        realDOM: node
      };
  ...
  function diffChildren(oldVChildren, newVChildren) {
    ...
    var _loop3 = function _loop3(i) {
      ...
      if (isArray(vOldNode)) {
        ...
        var newNodeDefinition = render(newVChildren[i]);
  ...
  function diffProps(oldProps, newProps) {
    var propsPatches = [];
    var updatedProps = {};
    Object.keys(newProps).filter(isProperty).forEach(function (key) {
    ...
    Object.keys(oldProps).filter(isProperty).forEach(function (key) {
      if (isEvent(key)) {
